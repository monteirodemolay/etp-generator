...
// Páginas que o Administrador pode liberar ou bloquear por usuário. "Entidades" e "Usuários"
// não entram aqui de propósito — são sempre exclusivas do papel Administrador.
...
const PAGINAS_PADRAO = Object.fromEntries(PAGINAS_CONFIGURAVEIS.map(p => [p.id, true]));

// Ações sobre documentos que o Administrador pode conceder ou revogar por usuário.
const ACOES_CONFIGURAVEIS = [
...
const ACOES_PADRAO = { criarDocumentos: true, editarDocumentos: true, excluirDocumentos: true, esvaziarLixeira: false };
...
// Uma entidade é "somente leitura" para o usuário quando ele tem acesso a ela, mas o
// Administrador marcou explicitamente que não pode gravar nada ali — só consultar.
function entidadeEhSomenteLeitura(usuario, entidadeId) {
...
export default function App({ emailUsuario = null }) {
  ...
  // Grava o ETP na nuvem (debounced) e mantém a lista local sincronizada. Um ETP recém-criado
  // só passa a existir de fato — na lista "Meus ETPs" e no banco — na primeira alteração real
  // que o usuário fizer; abrir e sair sem mexer em nada não deixa rascunho nenhum para trás.
  const persist = useCallback((etp) => {
    ...
    setEtps(prev => prev.some(x => x.id === etp.id)
      ? prev.map(x => (x.id === etp.id ? etp : x))
      : [etp, ...prev]);
    setSaveState("saving");
  ...
  function secretariaParaNovoDoc() {
    ...
    // Com o filtro em "todas", prefere uma entidade em que o usuário PODE gravar —
    // senão o documento nasceria numa entidade "só ver" e nunca seria salvo.
    const gravavel = secretariasVisiveis.find(s => !entidadeEhSomenteLeitura(usuarioAtual, s.id));
    return gravavel || secretarias[0];
  }

  // Recebe o que foi preenchido na janela de criação: objeto, tipo, processo e secretaria.
  // Importante: NÃO grava nem entra em "Meus ETPs" ainda — só na primeira alteração real feita
  // no editor (via persist()). Abrir e sair sem editar nada não deixa rascunho para trás.
  ...
  async function deleteEtp(id, e) {
    ...
    if (!doc) return;
    if (!permissoes.excluirDocumentos || entidadeEhSomenteLeitura(usuarioAtual, doc.secretariaId)) return;
    try {
  ...
  function duplicarEtp(etp, e) {
    e?.stopPropagation();
    if (!permissoes.criarDocumentos || entidadeEhSomenteLeitura(usuarioAtual, etp.secretariaId)) return;
  ...
  // Mesma regra dos ETPs: só grava e aparece na lista na primeira alteração real.
  ...
  function duplicarJustificativa(doc, e) {
    e?.stopPropagation();
    if (!permissoes.criarDocumentos || entidadeEhSomenteLeitura(usuarioAtual, doc.secretariaId)) return;
  ...
  function salvarJustificativa(doc) {
    ...
    setJustificativas(prev => prev.some(d => d.id === atualizado.id)
  ...
  function salvarDeclaracao(doc) {
    ...
    setDeclaracoes(prev => prev.some(d => d.id === atualizado.id)
      ? prev.map(d => (d.id === atualizado.id ? atualizado : d))
      : [atualizado, ...prev]);
...
function ListaDocumentos({ titulo, docs, onAbrir, onExcluir, onDuplicar, onNovo, icone: Icone, vazio, secretarias, mostrarSecretaria, podeCriar = true, podeExcluir = true }) {
